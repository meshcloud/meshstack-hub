...
| ---- | ----------- | ---- | ------- | :------: |
| <a name="input_git_ref"></a> [git\_ref](#input\_git\_ref) | meshstack-hub reference used to source the nested foundation, network-area, and network integration modules. `const` so it can be interpolated into the module source at init time. | `string` | `"main"` | no |
...
| <a name="input_hub_transfer_network"></a> [hub\_transfer\_network](#input\_hub\_transfer\_network) | IPv4 CIDR range used as the transfer network between the hub network area and connected networks. | `string` | `"10.0.255.0/24"` | no |
