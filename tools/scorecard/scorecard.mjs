...
const detectors = [
  ...
  },
  {
    ...
    fn: (mod) => {
      ...
      const federates = /data\s+"meshstack_building_block_runner"/.test(content) || /workload_identity_federation\s*=/.test(content);
      if (!federates) return { pass: null, detail: "module does not federate" };
      if (!/data\s+"meshstack_building_block_runner"/.test(content))
        return { pass: false, detail: "no meshstack_building_block_runner data source — issuer and audience describe the runner and are read from it" };
      return {
  ...
  },
  {
    ...
    fn: (mod) => {
      ...
      // from the runner rather than hardcoded or borrowed from another integration.
      const runnerWif = "data\\.meshstack_building_block_runner\\.[\\w-]+\\.spec\\.workload_identity_federation";
      return {
        pass:
          new RegExp(runnerWif + "\\.issuer").test(content) &&
          new RegExp(runnerWif + "\\.aws\\.audience").test(content),
        detail: "issuer and aws.audience do not come from the meshstack_building_block_runner data source",
      };
    },
  },
  {
  ...
  },
  {
    ...
    fn: (mod) => {
      ...
      return {
        pass: /google\.subject\s*==/.test(allTf) && /var\.workload_identity_federation_subjects/.test(allTf),
        detail: "attribute_condition does not compare google.subject to each var.workload_identity_federation_subjects entry",
      };
    },
  },
  {
