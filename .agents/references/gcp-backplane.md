...
`issuer`, `audience` and each subject come from a definition's resolved status, never hardcoded.
See
...
2. **What the pool provider trusts lives in its own variable.** Terraform tracks a dependency on a
   ...
   definition's status inside `workload_identity_federation` would put the pool, the
