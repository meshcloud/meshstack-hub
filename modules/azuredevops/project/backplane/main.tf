...
# Create a service principal for Azure DevOps management
resource "azuread_application" "azure_devops" {
  display_name = var.service_principal_name
  description  = "Service principal for managing Azure DevOps projects and users"
}
...
# Create a Key Vault to store the Azure DevOps PAT
...
# Custom role definition for Azure DevOps management
resource "azurerm_role_definition" "azure_devops_manager" {
  name        = "${var.service_principal_name}-manager"
  description = "Allows management of Azure DevOps projects and user entitlements"
  scope       = var.scope
