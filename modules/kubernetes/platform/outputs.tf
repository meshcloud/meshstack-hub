# Both token secrets wait for Kubernetes to populate them (wait_for_service_account_token), so
# these outputs are reliably non-empty on the creating apply — the composing architecture feeds
# them straight into the meshstack_platform replication and metering credentials.
