# Targets the cluster whose kubeconfig arrives as var.kubeconfig. The config reads from a variable
# (not a resource created here), so it is known at plan time.
provider "kubernetes" {
