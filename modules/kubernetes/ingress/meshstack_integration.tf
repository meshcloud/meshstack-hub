...
variable "acme_email" {
  ...
  description = "Contact address Let's Encrypt uses for expiry warnings and account recovery. Set on the definition, not asked at order time — see the input's comment."
}
...
resource "meshstack_building_block_definition" "this" {
  ...
  version_spec = {
    ...
    inputs = {
      ...
      # configures the kubernetes/helm providers, so it must be a concrete value (the cluster
      # already exists in a preceding building block).
      kubeconfig = {
      ...
      haproxy_service_annotations = {
        ...
        type            = "CODE"
        assignment_type = "STATIC"
        # jsonencode twice is correct, see the meshstack provider's building_block_definition docs.
        argument = jsonencode(jsonencode(var.haproxy_service_annotations))
      }

      # `dns01` is deliberately absent. The module can issue a wildcard certificate over DNS-01, but
      # that needs a DNS zone and a credential for it, and no hub module on this branch produces
      # either yet — so the building block leaves the input at its null default and certificates are
      # issued per hostname over HTTP-01. Add the input here once a DNS module exists.
    }
