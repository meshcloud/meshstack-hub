...
variable "haproxy_replica_count" {
  type        = number
  nullable    = false
  description = "Number of HAProxy ingress controller replicas. A value of 1 is sized for a demonstration cluster and gives no redundancy: every restart or node drain interrupts ingress traffic. Production wants at least 2, spread over separate nodes."
}
...
variable "dns01" {
  ...
  validation {
    # Both `try`s swallow the same single error: reading an attribute the caller left out of the
    # object, which `optional()` leaves absent rather than null. Absent means "not this provider".
  ...
  validation {
    # try: swallows the absent `certificate_domain` attribute, which means "default to the zone".
