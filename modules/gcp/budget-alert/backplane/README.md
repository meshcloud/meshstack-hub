...
`issuer` and `audience` come from `data.meshstack_building_block_runner`, each subject from the
resolved status of a building block definition — see `meshstack_integration.tf`.
