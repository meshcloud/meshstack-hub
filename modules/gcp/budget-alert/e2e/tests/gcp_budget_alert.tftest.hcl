run "building_block_gcp_budget_alert" {
  assert {
    condition     = meshstack_building_block.this.status.status == "SUCCEEDED"
    error_message = "gcp budget-alert building block expected SUCCEEDED, got ${meshstack_building_block.this.status.status}"
  }
  ...
  assert {
    ...
    error_message = "gcp budget-alert building block expected budget_id under the fixtures billing account, got ${jsondecode(meshstack_building_block.this.status.outputs["budget_id"].value)}"
  }
  ...
  assert {
    ...
    error_message = "gcp budget-alert building block expected budget_url to link the budget in the GCP console, got ${jsondecode(meshstack_building_block.this.status.outputs["budget_url"].value)}"
  }
  ...
  assert {
    ...
    error_message = "gcp budget-alert building block expected notification_channel_id in the backplane project, got ${jsondecode(meshstack_building_block.this.status.outputs["notification_channel_id"].value)}"
  }
  ...
  assert {
    ...
    error_message = "gcp budget-alert building block expected summary to name the requested budget, got ${jsondecode(meshstack_building_block.this.status.outputs["summary"].value)}"
  }

  assert {
    condition     = strcontains(jsondecode(meshstack_building_block.this.status.outputs["summary"].value), "**Monthly budget**: 100000 EUR")
    error_message = "gcp budget-alert building block expected summary to report the requested budget amount, got ${jsondecode(meshstack_building_block.this.status.outputs["summary"].value)}"
  }
  ...
  assert {
    ...
    error_message = "gcp budget-alert building block expected summary to report the requested threshold, got ${jsondecode(meshstack_building_block.this.status.outputs["summary"].value)}"
  }
}
