...
- **cert-manager** for TLS certificate management (the Let's Encrypt `ClusterIssuer` is installed by
  the separate `ske/cluster-issuer` building block, which runs after cert-manager's CRDs exist).
