...
variable "roles" {
  ...
  created in. Assigned at folder scope, where these roles are valid (organization scope rejects the
  service roles). Unused when external_service_account is true.

  `editor`, not `ske.admin`: the two carry the same `ske.cluster.*` permissions, but only `editor`
  also holds `service-enablement.service-state.edit`, which enabling SKE on a fresh project needs.
  See the backplane's `roles` variable for the full reasoning.
  EOT
}
