...
resource "meshstack_building_block_definition" "this" {
  ...
  spec = {
    ...
    readme = coalesce(var.bbd_readme, chomp(<<-EOT
      ...
      Authenticates to STACKIT through workload identity federation: the run exchanges the token
      meshStack injects for the service account given as the `STACKIT_SERVICE_ACCOUNT_EMAIL` input,
      so no key is stored anywhere. The composing architecture creates that service account and
      federates this definition into it; it needs permission to manage SKE in the target project.

      ## 📊 Shared responsibility

      | Responsibility | Platform Team | Application Team |
      |---|:---:|:---:|
      | Provide the STACKIT project, federated service account and cluster sizing | ✅ | ❌ |
  ...
  version_spec = {
    ...
    inputs = {
      # The composing architecture supplies the identity, and federates this definition into it by
      # ordering the STACKIT Service Account building block with this definition's uuid.
      ...
      kubernetes_version_min = {
        ...
        description     = "Minimum Kubernetes minor version to run."
        type            = "STRING"
        assignment_type = "STATIC"
