...
|------|-------------|------|---------|:--------:|
| <a name="input_building_block_definition_version_refs"></a> [building\_block\_definition\_version\_refs](#input\_building\_block\_definition\_version\_refs) | n/a | <pre>map(object({<br/>    kind = string<br/>    uuid = string<br/>  }))</pre> | n/a | yes |
...
| <a name="input_git_repository_template_repo_path"></a> [git\_repository\_template\_repo\_path](#input\_git\_repository\_template\_repo\_path) | Template repository path (owner/name) used for starterkit git repository creation. | `string` | n/a | yes |
...
| <a name="input_project_tags"></a> [project\_tags](#input\_project\_tags) | Tags for dev/prod meshProject. | <pre>object({<br/>    dev : map(list(string))<br/>    prod : map(list(string))<br/>  })</pre> | n/a | yes |
