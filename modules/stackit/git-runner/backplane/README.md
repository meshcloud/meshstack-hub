# STACKIT Git Runner — Backplane

Provisions the automation identity the Git Runner building block runs as: a STACKIT service
...
- A project-scoped `editor` role assignment, the narrowest built-in role carrying IaaS write
  (server, network, network interface, key pair).
...
- The STACKIT Git PAT the building block uses to mint runner registration tokens is **not** created
  here. It is a Git credential supplied to the building block as a static input by the platform team
