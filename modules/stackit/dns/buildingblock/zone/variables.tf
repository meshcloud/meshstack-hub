...
variable "dns_service_account_name" {
  ...
  validation {
    condition     = var.dns_service_account_name == null ? true : length(var.dns_service_account_name) <= 20
    error_message = "dns_service_account_name must be at most 20 characters. STACKIT rejects a longer name with \"Attribute name string length must be at most 20\"."
  }
}
