# Read off the zone resource whenever this module creates it, so that a caller wiring the name into
# something else — a building block definition naming the zone, an ExternalDNS filter — depends on
# the zone existing. Reading `var.zone_name` instead would hand out the same string with no edge in
# the graph behind it. With `create_zone = false` there is no such resource and the variable is all
# there is; it is also what carries `null` through when the module is switched off.
output "zone_name" {
  value       = var.create_zone ? one(stackit_dns_zone.this[*].dns_name) : var.zone_name
