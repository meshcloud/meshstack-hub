...
# Harbor takes the email as the basic-auth username and the token as the password.
jq -n \
  --arg access_token "${access_token}" \
  --arg service_account_email "${STACKIT_SERVICE_ACCOUNT_EMAIL}" \
  '{access_token: $access_token, service_account_email: $service_account_email}'
