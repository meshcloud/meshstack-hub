...
variable "meshstack" {
  ...
  description = "Shared meshStack context. Tags are propagated to the building block definition and to the landing zone created by the building block."
}
...
resource "meshstack_building_block_definition" "this" {
  ...
  spec = {
    ...
    Use this building block when a platform team wants to onboard STACKIT as a self-service platform in meshStack
    without assembling the location, folder and project platform by hand. After it runs, application teams can
    request STACKIT projects through the created landing zone.
    ...
    The building block authenticates to STACKIT with a service account key you supply as a secret file input. The
    service account needs `resource-manager.admin` on the organization so it can create the folder and projects. The
    key is written to a file and exposed to the STACKIT provider via `STACKIT_SERVICE_ACCOUNT_KEY_PATH`.
  ...
  version_spec = {
    ...
    inputs = {
      ...
      "stackit_service_account_key" = {
        ...
        description     = "STACKIT service account key JSON with `resource-manager.admin` on the organization. Will be used to setup all resources required for providing a STACKIT platform. The platform will use a dedicated service account with restricted permissions."
        type            = "CODE"
      ...
      tags = {
        display_name    = "Tags"
        description     = "Tags propagated to the created landing zone and building blocks."
        type            = "CODE"
        assignment_type = "STATIC"
