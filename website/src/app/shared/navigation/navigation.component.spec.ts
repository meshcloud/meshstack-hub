...
describe('NavigationComponent', () => {
  ...
  it('should highlight the active tab based on activeTab property', () => {
    ...
    expect(activeTab.nativeElement.textContent.trim()).toBe('AWS');
  });

  it('should have correct routerLink for each tab', () => {
    const links = fixture.debugElement.queryAll(By.css('.nav-link'));
    expect(links[0].attributes['ng-reflect-router-link']).toBe('/all');
    expect(links[1].attributes['ng-reflect-router-link']).toBe('/aws');
    expect(links[2].attributes['ng-reflect-router-link']).toBe('/azure');
    expect(links[3].attributes['ng-reflect-router-link']).toBe('/gcp');
  });
});
